    ...
    .then(function (sharesResult) {
      if (sharesResult.ocs.meta.statuscode === 200) {
        ...
        for (var shareI = 0; shareI < shares.length; shareI++) {
          ...
          if (share.share_type === 3 && share.id >= lastFoundShareId) {
...
Then(
  'a link named {string} should be listed as {string} in the public link list of file/folder/resource {string} on the webUI',
